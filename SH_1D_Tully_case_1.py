...
def D_12(c, d, x):
    v = -2*d*c*x*np.exp(-d*(x**2))
    return v


def U_i(a,b,c,d,x):
    en_1 = -np.sqrt((V_11(a, b, x))**2 + (V_12(c, d, x))**2)
    return en_1
...
def U_j(a,b,c,d,x):
    en_2 = np.sqrt((V_11(a, b, x))**2 + (V_12(c, d, x))**2)
    return en_2
...
#    hop_ji_LZ =np.exp(((-2*np.pi*(V_12(c,d,x))**2)/(2*D_11(a,b,x)))*v)
#    return hop_ji_LZ
