...
x = float(-4) #suggested x=0
v = float(0.3) #suggested v!=0
c_i = float(1)
c_j = float(0)
...
hop_ji_LZ = 0.0
a_HO_t = 0.0
u_ij = np.array([[ V_11(a, b, x), V_12(c, d, x)], [V_12(c, d, x),-V_11(a, b, x)]])
#u_ij = np.array([U_i(a,b,c,d,x), U_j(a,b,c,d,x)])
vk = np.array([nac_ij(a,b,c,d,x), -nac_ij(a,b,c,d,x)])
I = np.identity(len(vk))
H = np.zeros([len(vk),len(vk)])
I_rot = I[::-1]
...
#    hopp_LZ.append(hop_ji_LZ)
    ...
    u_ij_dt = np.array([[ V_11(a, b, x_dt), V_12(c, d, x_dt)], [V_12(c, d, x_dt),-V_11(a, b, x_dt)]])
    vk_dt = np.array([nac_ij(a,b,c,d,x_dt), nac_ij(a,b,c,d,x_dt)])
    
    H_dt = 1j*(u_ij_dt) + ((v*vk_dt)*I_rot)
    
    # Averagez Hamiltonian
    H_to = 0.5*(H + H_dt)
