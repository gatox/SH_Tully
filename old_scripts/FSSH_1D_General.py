...
"""Tuning factor for the coupling strength"""
F = 20.0
...
while(t <= t_max):
    ...
    if rep == 1:
        H_MCH = np.diag(Tully._energy(x_0)) #adiabatic representation at t_0
    else:
        H_MCH = Tully._di_energy(x_0)       #diabatic representation at t_0
        #F = 0.0
    ...
    if rep == 1:
        H_MCH_dt = np.diag(Tully._energy(x_dt)) #adiabatic representation at t_0 + dt
    else:
        H_MCH_dt = Tully._di_energy(x_dt)       #diabatic representation at t_0 + dt
        #F = 0.0
...
plt.plot(time,np.array(norm)[:,1], label = '$|C_j(t)|^2$')
plt.plot(time,np.array(norm)[:,0], label = '$|C_i(t)|^2$')
plt.xlabel('Time (fs)', fontweight = 'bold', fontsize = 16)
