...
class VelocityVerletPropagator:
    ...
    def __init__(self, state):
        ...
        if isscalar(state.mass) and isscalar(state.vel): 
            if self.state.crd < 0:
                self.x_min = self.state.crd
    ...
    def run(self):
        ...
        if self.state.model != 1: 
            if self.t > self.t_max:
                raise SystemExit("Noting to be done")
        else:
            if (self.x_min > self.state.crd or self.state.crd > self.x_max):
                raise SystemExit("wrong sign of the coordinate variable")
