...
class Propagator:
    ...
    def probabilities_tully(self, state, vk_old, ene_new, dt):
        ...
        mch = self.mch_propagator(ene, vk, dt)
        u_diab = mch.u
        p_mch = mch.p_mch 
        probs = (2.0 * imag(rho_old[instate,:] * h_total[:,instate]) * dt)/(real(rho_old[instate,instate])) 
        probs[instate] = 0.0
        probs = maximum(probs, 0.0)
        tully = namedtuple("tully", "probs rho_old p_mch u_diab")
        return tully(probs, rho_old, p_mch, u_diab) 
    ...
    def diag_propagator(self, ene_cou_grad, dt, state):
        ...
        mch_new = self.mch_propagator(ene, vk, dt)
        p_mch_new = mch_new.p_mch 
    ...
    def new_prob_grad(self, state, vk_old, ene_cou_grad, dt):
        if self.prob_name == "diagonal":
            ...
            result = namedtuple("result","probs grad_new tully u_diab") 
            return result(probs, grad_new, tully, u_diab)
        else:
...
class SurfaceHopping(BornOppenheimer):
    ...
    def get_ene_cou_grad(self, crd, curr_state):
        ...
        if isinstance(h_mch, ndarray) != True: 
            h_mch = array(h_mch)
        if h_mch.ndim == 1:
            rep = "adiabatic"
            ...
            rep = "diabatic"
            ene, u = linalg.eigh(h_mch)
        grad = self.get_gradient(crd, curr_state) 
        if self.coupling == "nacs":
            nac = self.get_coupling(crd)
            ene_cou_grad = namedtuple("ene_cou_grad", "ene u nac grad rep")
            return ene_cou_grad(ene, u, nac, grad, rep)
        elif self.coupling == "wf_overlap":
            wf_ov = self.get_coupling(crd)
            ene_cou_grad = namedtuple("ene_cou_grad", "ene u wf_ov grad rep")
            return ene_cou_grad(ene, u, wf_ov, grad, rep)
    ...
    def new_surface(self, state, results, crd_new, t, dt):
        ...
        state.ene = ene_cou_grad.ene
        state.epot = state.ene[state.instate]
        results.save_db(t, ene_cou_grad.u, ene_cou_grad.rep, state) #save variables in database
...
class PrintResults:
    ...
    def diab_population(self, nstates, u, currstate):
        diab = zeros(nstates)
        diab[currstate] = 1
        pop_diab = linalg.multi_dot([u.T.conj(), diag(diab), u])     
        return diag(pop_diab.real)

    def save_db(self, t, u_diab, rep, state):
        ...
        if state.method == "Surface_Hopping":
            ...
            if rep == "adiabatic":
                ...
                db.set("fosc",self.diab_population(nstates, u_diab, state.instate))
