...
class OpenMolcas(AbinitioBase):
    ...
    [noise(yes)]
    mean = 0 :: float
    standar_d = 0.01 :: float
    [noise(not)]
    add_noise = false :: bool
    """
    ...
    def __init__(self, config, atomids, nstates, charge, spin, exe, basis, couplings, caspt2, f_osc, noise):
        ...
        self.noise_mean = config['noise']['mean']
        self.noise_sd = config['noise']['standar_d']
        self.basis = basis 
        self.icall = 0
    ...
    def from_config(cls, config, atomids, nstates, nghost_states):
        return cls(config, atomids, nstates, config['charge'], config['spin'], config['exe'], config['basis'], config['couplings'], config['caspt2'], config['f_osc'], config['noise'])
    ...
    def _read_energies(self, output, key):
        ...
        ene = copy(db[key])
        return self._check_noise(ene[:self.nstates]) 
    ...
    def _read_xms_energies(self, output):
        ...
        if not isinstance(out['XMS_Energies_OpMol'], list):
            ...
            ene = out['XMS_Energies_OpMol']
        return self._check_noise(np.array(ene))  
    ...
    def _read_nacs(self, output):
        ...
        if self.nstates > 2:
            for i in range(leng):
                nac_i = self._check_noise(np.array(nac[i]))
                nacs.update({(int(out['NACs_Index_OpMol'][i][0]-1),int(out['NACs_Index_OpMol'][i][1]-1)):nac_i})
                nacs.update({(int(out['NACs_Index_OpMol'][i][1]-1),int(out['NACs_Index_OpMol'][i][0]-1)):-nac_i})
        elif self.nstates == 2:
            nac_0 = self._check_noise(np.array(nac[0]))
            nacs.update({(int(out['NACs_Index_OpMol'][0][0]-1),int(out['NACs_Index_OpMol'][0][1]-1)):nac_0})
            nacs.update({(int(out['NACs_Index_OpMol'][0][1]-1),int(out['NACs_Index_OpMol'][0][0]-1)):-nac_0})
        else:
    ...
    def _read_grads(self, state):
        ...
        if not isinstance(out['Gradient_OpMol'][0][0], list):
            ...
            grad = out['Gradient_OpMol']
        return self._check_noise(np.array(grad)) 
    ...
    def submit(self, filename): 
        ...
        f = "-f"
        cmd = f"$MOLCAS/{self.exe} {filename} {f}"
        os.system(cmd)
...
if __name__=='__main__':
    ...
    res = spp.request(crd, ['energy','gradient','nacs'], states=[1])
