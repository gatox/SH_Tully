...
#    remsection = :: literal
    ...
    def __init__(self, config, atomids, nstates, chg, mult, qchemexe, couplings, spin_flip, basis, method):
    ...
    def from_config(cls, config, atomids, nstates):
        return cls(config, atomids, nstates, config['chg'], config['mult'], config['exe'], config['couplings'], config['spin_flip'], config['basis'], config['method'])
    ...
    def submit_save(self, filename): 
        ...
        save_file = filename 
        save = "-save"
        cmd = f"{self.exe} {save} {filename} {output} {save_file} > save"
        os.system(cmd)
        return output
