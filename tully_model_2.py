...
#class Tully_2(Colt):
    ...
    def __init__(self, a,b,c,d, e0):
        """ Tully model 2: Simple avoided crossing 
    ...
    def from_config(cls,config):
        return cls(config['a'], config['b'], config['c'], config['d'], config['e0'])

    
    def _di_energy(self, x):
        """ returns the matrix of diabatic energies at position x """
        V_11 = 0.0
        V_22 = -self.a*np.exp(-self.b*(x**2)) + self.e0
    ...
    def _di_gradient(self, x):
        ...
        D_11 = 0.0
        D_22 = self.b*(2*x)*self.a*np.exp(-self.b*(x**2))
    ...
    def _n_coupling(self, x):
        ...
        dij= np.dot(coeff[:,0].Tcoeff.T, np.dot(DV, coeff[:,1]))
        dE = energies[1] - energies[0]
        return np.array([ [0, -dij/(12*dE)],
                          [dij/(12*dE), 0 ] ])
...
#    HO = Tully_2(a = 0.10, b = 0.28, c = 0.015, d = 0.06, e0 = 0.05)
#    request = namedtuple('request', 'crd energys gradients coupling')
#    request.crd = np.array(-2)
...
a = SurfacePointProvider.from_questions(["energy"], 2, 1, config ="Tully2.in")

#a = Tully_2.from_questions(config = "values")
